...
export default function Requests() {
  ...
  return (
    <div className="page-container bg-app min-h-screen">
      ...
      {loading && <p className="text-grayText">Loading requests...</p>}

      {!loading && landlordRequests?.length === 0 && (
        <p className="text-grayText mt-3">No incoming requests yet.</p>
      )}
          ...
          >
            ...
            {req.status === "requested" && (
              <div className="flex gap-3 mt-5">
                ...
                <button
                  onClick={() => handleStatusUpdate(req._id, "rejected")}
                  className="flex-1 btn-danger-outline py-2 rounded-xl text-sm"
                >
                  Reject
                </button>
              </div>
            ...
            {req.status === "approved" && (
              <p className="text-primaryDark mt-4 text-sm font-medium">
                ✔ Approved — waiting for tenant deposit
              </p>
            )}
